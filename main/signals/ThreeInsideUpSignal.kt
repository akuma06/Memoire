...
class ThreeInsideUpSignal() : ChartSignal() {
    ...
    override fun update(asset: OrderBook, day: Day): Signal.Direction {
        ...
        if (prices[t-3].first < prices[t-3].last)
            return Signal.Direction.HOLD
        // le prix open t-2 doit être inférieur au open précédent mais supérieur au close précédent
        if (prices[t-2].first > prices[t-3].first || prices[t-2].first <= prices[t-3].last)
            return Signal.Direction.HOLD
        // le prix close t-2 doit être inférieur au open précédent mais supérieur au close précédent
        if (prices[t-2].last >= prices[t-3].first || prices[t-2].last < prices[t-3].last)
            return Signal.Direction.HOLD
        ...
        // Si on est toujurs là, ça veut dire qu'on est dans le pattern donc tendance haussiere
